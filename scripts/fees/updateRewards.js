...
function getOptimismValues() {
  ...
  const TRADE_TOTAL_REBATE_USD = "0"
  const LIQUIDITY_TOTAL_REBATE_USD = "2996667000000000000000000000000"
...
function getBaseValues() {
  ...
  const decimals = tokens.usdbc.decimals;
...
async function main() {
  const feeToken = network === "base" ? tokens.usdbc : tokens.usdc
  const rewardToken = await contractAt("Token", feeToken.address)
